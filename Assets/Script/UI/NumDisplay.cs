...
public class NumDisplay : MonoBehaviour
{
    ...
    public void GenerateUINum(int num, float size,int color)
    {
        ...
        else
        {
            ...
            if(color == 0)
            {
                GameObject obj = Instantiate(uinum_w_obj[ten], new Vector3(parent_pos.x+0.1f, parent_pos.y, parent_pos.z), Quaternion.identity, parent);
                obj.transform.localScale = new Vector3(size, size, 1);
                save.Add(obj);

                GameObject obj2 = Instantiate(uinum_w_obj[one], new Vector3(parent_pos.x - 0.1f, parent_pos.y, parent_pos.z), Quaternion.identity, parent);
